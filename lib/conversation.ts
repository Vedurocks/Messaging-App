...
export async function fetchConversationMessages(
  ...
  const messagesQuery = await sql`
    ...
    WHERE m.conversation_id = ${conversationId}
      ${cursor ? sql`AND m.created_at < (SELECT created_at FROM messages WHERE id = ${cursor})` : sql``}
...
async function expandThreadReplies(
  ...
  const repliesQuery = await sql`
    WITH RECURSIVE thread_tree AS (
      ...
      FROM messages m
      JOIN users u ON m.sender_id = u.id
      WHERE m.parent_message_id = ANY(${parentIds})
...
export async function markMessagesAsRead(
  ...
  await sql`
    ...
    SELECT unnest(${messageIds}::uuid[]), ${userId}::uuid
