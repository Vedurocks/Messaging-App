...
async function expandThreadReplies(
  ...
  const repliesQuery = await query(
...
export async function sendMessage(
  ...
  const node = rowToNode(fullMessage.rows[0]);

  // Real-time push. Best-effort: see lib/realtime.ts's documented caveat —
  // this only reaches subscribers connected to the same server instance.
  // The message is already persisted regardless; a broadcast failure here
  // must never fail the whole send.
  try {
  ...
  // Push notifications to other participants. Awaited (not fire-and-forget)
  // since this runs in a serverless function that may terminate once the
  // response is sent — but sendPushToUsers/sendPushToUser never throw, so
  // this can't fail the message send itself.
  try {
    const others = await sql`
      SELECT user_id FROM conversation_participants
      WHERE conversation_id = ${conversationId} AND user_id != ${senderId} AND left_at IS NULL
    `;
    const otherIds = others.rows.map((r) => r.user_id);
    if (otherIds.length > 0) {
      ...
      await sendPushToUsers(otherIds, {
        title: node.sender.displayName || node.sender.username,
        body: preview,
        url: `/?c=${conversationId}`,
      });
    }
  } catch {
    // non-fatal — push is a nice-to-have, never blocks message delivery
  }
